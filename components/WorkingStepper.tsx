        ...
        if (phoneRegex.test(part)) {
            ...
            return (
                <a
                    key={index}
                    href={`tel:${cleanPhone}`}
                    className="text-primary hover:opacity-80 underline font-medium transition-colors"
                >
                    {part}
                </a>
            );
    ...
    if (isEnabled) {
        return (
            <div className={`mb-12 ${className}`}>
                <h2 id="working-stepper-heading" className="text-3xl font-bold mb-6">{t.howToStartWorking}</h2>
                <ol className="list-decimal pl-6 space-y-6">
                    {steps.map((step, index) => (
                        <li key={index} className="text-xl pl-2">
                            <strong className="block mb-2">{t[step.titleKey]}</strong>
